...
namespace SchedulerApp.Services
{
    public class StateSave
    {
        ...
        private readonly string exportsFolder;
        ...
        public StateSave()
        {
            ...
            exportsFolder = Path.Combine(appDataFolder, "Exports");
        ...
        public void SaveEmployee(AvailabilityEntry employee)
        {
        ...
        public void SaveAllEmployees(List<AvailabilityEntry> employees)
        {
        ...
        public void SaveAllBatches(List<Batch> batches)
        {
            if (!ENABLE_SAVING) return;
        ...
        public List<AvailabilityEntry> LoadEmployees()
        {
            var employees = new List<AvailabilityEntry>();
            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    try
                    {
                        ...
                        var employee = JsonConvert.DeserializeObject<AvailabilityEntry>(json);
                        if (employee != null)
                        {
        ...
        public void SaveExport(ExportSchedule exportSchedule)
        {
            if (!ENABLE_SAVING || exportSchedule == null) return;

            try
            {
                string filePath = Path.Combine(exportsFolder, $"{exportSchedule.Id}.json");
                string json = JsonConvert.SerializeObject(exportSchedule, Newtonsoft.Json.Formatting.Indented);
                File.WriteAllText(filePath, json);
            }
            catch { }
        }

        public void SaveAllExports(List<ExportSchedule> exports)
        {
            if (!ENABLE_SAVING) return;
            ...
            try
            {
                ...
                foreach (var export in exports) SaveExport(export);
            }
            catch { }
        }

        public List<ExportSchedule> LoadExports()
        {
            var exports = new List<ExportSchedule>();
            if (!ENABLE_SAVING || !Directory.Exists(exportsFolder)) return exports;
            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        string json = File.ReadAllText(file);
                        var export = JsonConvert.DeserializeObject<ExportSchedule>(json);
                        if (export != null)
                        {
                            // Settings (colors/template) reset to defaults each session.
                            export.Settings = new ExportSettings();
                            exports.Add(export);
                        }
                    }
                    catch { }
        ...
        public void DeleteExport(string exportId)
        {
            ...
            try
            {
                string filePath = Path.Combine(exportsFolder, $"{exportId}.json");
                if (File.Exists(filePath)) File.Delete(filePath);
            }
            catch { }
        }

        public void CleanupOrphanedEmployees(List<Batch> allBatches)
        {
            if (!ENABLE_SAVING) return;
